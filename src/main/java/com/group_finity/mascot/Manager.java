...
import com.group_finity.mascot.exception.BadRequestException;
...
public class Manager {
     ...
     * Sets the {@link Behavior} for all {@link Mascot Mascots}.
     *
     * @param name the name of the {@link String}
     */

    public void setBehaviorAPI(final String name, String imageSet, ArrayList<Integer> shimejiId) {
        synchronized (getMascots()) {
            if (imageSet == null && shimejiId == null){ //Set for all
                for (final Mascot mascot : getMascots()) {
                    try {
                        if (behavioursValidation(mascot, name)) mascot.setBehavior(name);
            ...
            } else if(imageSet != null){    //Set for all with the imageSet
                for (final Mascot mascot : getMascots()) {
            ...
            } else if(!shimejiId.isEmpty()){    //Set to a specific Shimeji ID
                for (final Mascot mascot : getMascots()) {
                    if (shimejiId.contains(mascot.getId())) {
                        try {
                            if(behavioursValidation(mascot, name)) mascot.setBehavior(name);
                        } catch (final CantBeAliveException e) {
                            log.log(Level.SEVERE, "Failed to set behavior to \"" + name + "\" for mascot \"" + mascot + "\"", e);
    ...
    private Boolean behavioursValidation(Mascot mascot, String behaviourName) {
        ArrayList<String> behavioursList = new ArrayList<>();

        Configuration config = VShimejiApplication.getInstance().getConfiguration(mascot.getImageSet());
        behavioursList.addAll(config.getBehaviorNames());

        return behavioursList.contains(behaviourName);
    }
