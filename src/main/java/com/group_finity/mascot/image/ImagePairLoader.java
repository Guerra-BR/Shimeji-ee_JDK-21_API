...
public class ImagePairLoader {
    ...
    public static void load(final String name, final String rightName, final Point center, final double scaling, final Filter filter) throws IOException {
        ...
        final BufferedImage leftImage = scale(premultiply(ImageIO.read(VShimejiApplication.IMAGE_DIRECTORY.resolve(name).toFile())), scaling, filter);
        final BufferedImage rightImage;
        if (rightName == null) {
            rightImage = flip(leftImage);
        } else {
            rightImage = scale(premultiply(ImageIO.read(VShimejiApplication.IMAGE_DIRECTORY.resolve(rightName).toFile())), scaling, filter);
        }
