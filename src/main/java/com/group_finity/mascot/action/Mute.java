...
public class Mute extends InstantAction {
    ...
    protected void apply() throws VariableException {
        ...
        if (soundName != null) {
            List<Clip> clips = Sounds.getSoundsIgnoringVolume(VShimejiApplication.SOUND_DIRECTORY.resolve(soundName).toString());
            if (clips.isEmpty()) {
                clips = Sounds.getSoundsIgnoringVolume(VShimejiApplication.SOUND_DIRECTORY.resolve(getMascot().getImageSet()).resolve(soundName).toString());
                if (clips.isEmpty()) {
                    clips = Sounds.getSoundsIgnoringVolume(VShimejiApplication.IMAGE_DIRECTORY.resolve(getMascot().getImageSet()).resolve(VShimejiApplication.SOUND_DIRECTORY).resolve(soundName).toString());
                    for (Clip clip : clips) {
